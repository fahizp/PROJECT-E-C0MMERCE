    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (order.couponName) {
          ...
          let couponIndex = await userData.coupon.findIndex(
            (dataFind) => dataFind.name == order.couponName
          );
          if (couponIndex >= 0) {
            let dbuserCoupenUpdate = await db.user.updateOne(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (order) {
          ...
          let productIndex = order[0].orders[
            orderIndex
          ].productDetails.findIndex((product) => product._id == `${proId}`);
          db.order
            .updateOne(
              { "orders._id": `${orderId}` },
              {
  ...
  editAddress: (data, id, userId) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (addressObj) {
          let addressIndex = addressObj[0].address.findIndex(
